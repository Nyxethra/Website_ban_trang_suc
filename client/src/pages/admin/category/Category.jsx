          ...
          <tbody>
            {data?.map((c, index) => (
              <tr key={c._id}>
                ...
                <th>{c.name}</th>
                <th>
                  <i className='fa fa-trash' type="button" style={{ color: "red" }} onClick={() => handleDelete(c._id)}></i>
                  <i className='fa fa-pencil mx-2 text-primary' type="button" onClick={() => handleUpdate(c._id)}></i>
                </th>
              </tr>
            ))}
          </tbody>
